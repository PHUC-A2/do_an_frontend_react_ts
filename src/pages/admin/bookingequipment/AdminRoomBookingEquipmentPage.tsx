...
import {
    getAllRoomBookingDevices,
    getAssetUsageById,
    updateRoomBookingDeviceStatus,
    adminConfirmRoomBookingDeviceReturn,
    adminGetRoomEquipmentBorrowLogs,
    adminGetRoomEquipmentUsageStats,
} from '../../../config/Api';
import {
...
const AdminRoomBookingEquipmentPage = () => {
    ...
    const handlePrintHandover = useCallback(
        async (record: IBookingEquipment) => {
            ...
            try {
                try {
                    const res = await getAssetUsageById(record.bookingId);
                    const usage: IAssetUsage | null = res.data.data ?? null;
                    if (usage && Number(res.data.statusCode) === 200) {
                        const startDateTime = `${usage.date}T${usage.startTime}`;

                        const endDateTime = `${usage.date}T${usage.endTime}`;
                        ...
                        const booking: IBooking = {
                            ...
                            userName: usage.userName ?? usage.userEmail ?? '—',

                            pitchId: usage.assetId,

                            pitchName: usage.assetName ?? '—',
                            ...
                            status:
                                ...
                                usage.status === 'CANCELLED' || usage.status === 'REJECTED' ? 'CANCELLED' : 'PAID',
    ...
    const reloadBorrowList = useCallback(async () => {
        const res = await getAllRoomBookingDevices();
    ...
    const reloadLogs = useCallback(async () => {
        setLogsLoading(true);
        try {
            const res = await adminGetRoomEquipmentBorrowLogs();
    ...
    const reloadStats = useCallback(async () => {
        ...
        try {
            const res = await adminGetRoomEquipmentUsageStats();
    ...
    const handleUpdateStatus = async (id: number, body: IUpdateBookingEquipmentStatusReq) => {
        setUpdatingId(id);
        try {
            const res = await updateRoomBookingDeviceStatus(id, body);
    ...
    const handleAdminConfirmReturn = async (id: number) => {
        setConfirmingId(id);
        try {
            const res = await adminConfirmRoomBookingDeviceReturn(id);
            if (Number(res.data.statusCode) === 200) {
...
export default AdminRoomBookingEquipmentPage;
